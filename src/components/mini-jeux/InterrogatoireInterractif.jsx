...
const InterrogatoireInterractif = ({onClose}) => {
  ...
  const data = [
    ...
    // id 1 - reponse intro
    {
      ...
      choices: [
        {
          label: "on nom est Timothée Lombardo, tu as 28 ans. Ancien hacker, tu as évité la prison grâce au boulot que t'as proposé Raphaëlle à la Détective Box, il y a 4 ans, c'est bien ça ?",
          target: 2
        }
      ]
    },
    // id 2 - identite Tim
    {
      ...
      choices: [
        ...
        {
          label: '(calme) En voulant débloquer les caméras qui sont toujours en boucle, on a remonté le hacking des casseurs et le nom "LONEWOOD" est apparu. Est-ce que tu pourrais nous en dire plus ?',
          target: 4
        }
      ]
    },
    // id 3 - brusque 1
    {
      ...
      choices: [
        {
          label: '(Direct) Ton nom apparaît clairement dans le hacking du système de sécurité du casino de ton père par La Horde, ça peut pas être un hasard. Dis-nous ce que tu sais, Tim.',
          target: 5
        },
        {
          label: '(Brusque) Mais arrête de te foutre de nous, on a trouvé ton nom en voulant débloquer les caméras de sécurité : LONEWOOD',
          target: 6
        }
      ]
    },
    // id 4 - calme 1
    {
      ...
      choices: [
        {
          label: "(Impatient) Ce serait pas la première fois qu'un membre de votre équipe fait des bêtises... Surtout en ce qui te concerne. Alors arrête de tourner autour du pot et dis-nous ce que tu sais sur le hacking de La Horde.",
          target: 5
        },
        {
          label: '(Compatissant) Je sais que tu fais tout pour être clean maintenant. Mais mets-toi à notre place, ça ne peut pas être une coïncidence.',
          target: 7
        }
      ]
    },
    ...
    // id 6 - brusque 2
    {
      ...
      choices: [
        ...
        {
          label: "(Curieux) Comment ça reprendre le nom de ton père ? Lonewood c'est ton nom de hacker ? Tu dis que tu t'es fait piégé ?",
          target: 10
        }
      ]
    },
    // id 7 - compatissant 1
    {
      ...
      choices: [
        {
          label: "(Incrédule) D'après ce que j'ai compris, tu détestes ton père et son attrait pour l'argent. Ca pourrait être un moyen pour toi de mettre un coup de pieds dans la fourmillière.",
          target: 11
        },
        {
    ...
    // id 9 - aggressif
    {
      ...
      choices: [
        {
          label: "(Las) Ok, on te croit Tim. Du coup tu penses que c'est pour se venger qu'il a mis ton nom dans le hacking ? Une idée de qui ça pourrait être ?",
          target: 12
        },
        {
    ...
    // id 10 - Curieux
    {
      ...
      choices: [
        ...
        {
          label: "(Perplexe) Mmh peut-être que c'était quelqu'un qui te connaissait à l'époque et qui a voulu se venger de quelque chose que tu lui as fait... une idée de qui ça pourrait être ?",
          target: 12
        }
      ]
    },
    // id 11 - Incrédule/Perplexe
    {
      ...
      choices: [
        ...
        {
          label: "(Méprisant) Mouais tu parles. Quoi qu'il en soit tu es responsable de tout ces casses. Rends-toi utile et peut-être que ça t'évitera la prison : qui est ce hacker qui t'as contacté ?",
          target: 15
        }
      ]
    },
  ...
  const renderChoices = choices => choices.map((choice, id) => 
    <button className="modal-objectif__button button--red" key={id} onClick={() => handleChoice(choice.target)}>
      {choice.label}
    </button>
  )
  return (
    ...
    <div className="modal-objectif__box">
      ...
      <h2 className="modal-objectif__title">
        Interrogatoire de Timothée Lombardo
      </h2>
