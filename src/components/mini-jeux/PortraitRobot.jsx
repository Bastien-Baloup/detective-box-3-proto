...
const PortraitRobot = ({ onValid }) => {
  ...
  const data = [
    ...
    { label: 'La mâchoire', values: ['Indéterminée', 'Arrondie', 'Carrée', 'Décalée'] },
    { label: 'Longueur des cheveux', values: ['Chauve', 'Courts', 'Mi-longs', 'Longs'] },
    { label: 'Couleur des cheveux', values: ['Bruns', 'Blonds', 'Châtains', 'Roux'] },
    { label: 'Les sourcils', values: ['Inexistants', 'Taillés', 'Broussailleux', 'Monosourcil'] },
    { label: 'Forme des yeux', values: ['Indéterminée', 'En amandes', 'Ronds', 'Mono-paupière'] },
    { label: 'Couleur des yeux', values: ['Indéterminée', 'Bleus', 'Verts', 'Marrons'] },
    { label: 'Le nez', values: ['Asymétrique', 'Cassé', 'Court', 'Long'] },
    { label: 'La bouche', values: ['Très fine', 'Fine', 'Moyenne', 'Pulpeuse'] },
      ...
      <form className='modal-objectif__form modal-objectif__portrait-robot' onSubmit={handleSubmit}>
        {data.map((champs, index) => (
          <div className='modal-objectif__select-container' key={index}>
            ...
            <select
              onChange={e => handleSelectChange(champs.label, e.target.value)}
            >
