...
const EventHandler = () => {
  ...
  const box1audio2 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio2'),
    [dataHistory]
  )
  const box1audio4 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio4'),
    [dataHistory]
  )
  const box1audio5 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio5'),
    [dataHistory]
  )
  const box1audio6 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio6'),
    [dataHistory]
  )
  const box1audio7 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio7'),
    [dataHistory]
  )
  const box1audio8 = useMemo(
    () => dataHistory && dataHistory.find(document => document.id == 'box1audio8'),
    [dataHistory]
  )
  ...
  const event131 = useMemo(() => dataEvent && dataEvent.find(event => event.id == 131)?.status, [dataEvent])
  const event132 = useMemo(() => dataEvent && dataEvent.find(event => event.id == 132)?.status, [dataEvent])
    ...
    return (
      <div className='modal-objectif__background'>
        <div className='modal-objectif__box'>
          ...
          {!box1audio6?.status && (
            <button className='modal-objectif__button button--red' onClick={clickHannah}>
              Hannah Evans
            </button>
          )}
          {!box1audio8?.status && (
            <button className='modal-objectif__button button--red' onClick={clickHenri}>
              Henri Leveaux
            </button>
          )}
          {!box1audio7?.status && (
            <button className='modal-objectif__button button--red' onClick={clickSacha}>
              Sacha Leza
            </button>
          )}
          {!(box1audio4?.status || box1audio5?.status) && (
            <button className='modal-objectif__button button--red' onClick={clickEllie}>
              Ellie Levyn
            </button>
          )}
          {allDone && (
            <button className='modal-objectif__button button--red' onClick={closeInterrogatoireCasseurs}>
              Continuer l&apos;enquête
            </button>
          )}
        </div>
